// USB HID core
//  Ver 8   2018/09/12  一部PCで接続するとフリーズする対策 キーボードデータの受信処理追加
//  Ver 7   2016/06/29  USB通信によるEEPROM書き換え要求方法を変更
//  Ver 6   2012/12/10  EEPROMの読み書きを複数行うように変更
//	Ver	5	Windows起動不可対応
//	Ver	4	USBキーボード、F13～F24にも対応
//	ver	3	ReviveUSB向けに変更
//	ver	2	マウスの移動に対応
//	ver 1	USB core最初のバージョン
...
//			eeprom_data[fi][fj] = ReadEEPROM(fi*3+fj) ;
			uc_temp = ReadEEPROM_Agree(EEPROM_SAVE_NUM*(fi*3+fj), EEPROM_SAVE_NUM, EEPROM_SAME_COUNT, &eeprom_data[fi][fj]);
		}
	}
...
//				WriteEEPROM(fi*3+fj,0x00);
				uc_temp = WriteEEPROM_Agree(EEPROM_SAVE_NUM*(fi*3+fj), 0x00, EEPROM_SAVE_NUM);
			}
		}
	}
}//end UserInit
...
//何も押されていないときの為にバッファを初期化しておく
	...
	{
		if(fi < 8)
			result_button_press = ((result_button_press_set & (0x01 << fi)) ? 1:0);
		else
			result_button_press = ((result_button_press_set2 & (0x01 << (fi-8))) ? 1:0);
    ...
    if(!HIDRxHandleBusy(USBOutHandle))				//Check if data was received from the host.
    {
        switch(ReceivedDataBuffer[0])
        {
            case 0x80:  // EEPROMにデータを設定
				if(ReceivedDataBuffer[37] == 0x05
					&& ReceivedDataBuffer[38] == 0xAA
					&& ReceivedDataBuffer[39] == 0x55
					&& ReceivedDataBuffer[40] == 0x0A
					&& ReceivedDataBuffer[41] == 0x0F)
				{
					for(fi = 0;fi < NUM_OF_PINS; fi++)
					{
						for(fj = 0;fj < 3;fj++)
						{
			            	eeprom_data[fi][fj] = ReceivedDataBuffer[1+(fi*3)+fj];
							uc_temp = WriteEEPROM_Agree(EEPROM_SAVE_NUM*((fi*3)+fj), eeprom_data[fi][fj], EEPROM_SAVE_NUM);
//							WriteEEPROM(fi*3+fj,eeprom_data[fi][fj]);
            ...
            case 0x81:  //ボタンの押下状態を返信
				...
				ToSendDataBuffer[1] = result_button_press_set & 0xff;
				ToSendDataBuffer[2] = result_button_press_set2 & 0x0f;
